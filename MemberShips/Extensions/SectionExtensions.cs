using MemberShipsPro.Comparers;
...
namespace MemberShipsPro.Extensions
{
            ...
            try
            {
                ...
                var items = await (from i in db.Items
                                   ...
                                   select new ProductItemRow
                                   {
                                       ...
                                       Title = i.Title,
                                       Link = it.Title.ToUpper().Contains("PDF") ? i.Url : "/ProductContent/Content/" + pi.ProductId + "/" + i.Id,
                                       Url = i.Url,
                                       ImageUrl = i.ImageUrl,
                                       ...
                                       IsAvailable =true,
                                       IsDownload = it.Title.ToUpper().Contains("PDF")
                                   }).ToListAsync();
                
                foreach (ProductItemRow row in items)
                {
                    row.IsDownload = row.Title.ToUpper().Contains("PDF");
                    row.Link = row.Title.ToUpper().Contains("PDF") ? row.Url : row.Link ;
                }
